...
const ChatAltIcon = () => h('svg', { class: 'h-5 w-5', fill: 'none', stroke: 'currentColor', viewBox: '0 0 24 24' }, [
...
const notificationSummary = computed(() => {
  if (notificationsLoading.value) return 'Checking salary and class reminders'
  if (notifications.value.length === 0) return 'Everything is clear for now'
  const salary = notifications.value.filter(item => item.type === 'Salary').length
  const attendance = notifications.value.filter(item => item.type === 'Attendance').length
  return `${salary} salary update${salary === 1 ? '' : 's'}, ${attendance} attendance reminder${attendance === 1 ? '' : 's'}`
})

const notificationTone = (type: string) => {
  if (type === 'Salary') return 'bg-amber-400'
  if (type === 'Attendance') return 'bg-emerald-400'
  return 'bg-slate-300'
}
