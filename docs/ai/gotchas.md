...
**Root Cause**: Admin list endpoints filter by `SessionId`, but a stale selected ID can hide valid data. Broadly backfilling every legacy `NULL Student.SessionId` to the active session is unsafe because it collapses historical students into the current session. A repair can also hide valid students if it clears an explicit `Student.SessionId` merely because the student has no class mapping.
**Solution**: Validate incoming session IDs and make the frontend replace stale selections. For Students, preserve every existing explicit session assignment, reconstruct missing ownership from the most recent active class enrollment, then recover only still-unmapped students whose admission date belongs to exactly one academic session. Never clear a valid session solely because a class mapping is absent, and never bulk-assign all students to the active session. Pass the selected session explicitly when listing or creating students, and synchronize `Student.SessionId` when mapping a class. On legacy databases with empty EF migration history, run repairs through the compatibility path behind durable one-time repair markers.
...
**Solution**: Execute the entire atomic payment, generated fallback reference, receipt save, and commit inside the context's execution strategy. Configure the SQL Server regression test with `EnableRetryOnFailure` so the Azure behavior is exercised locally. Legacy databases can then expose `DB-207` or `DB-208` because their receipt tables predate the current EF columns or are absent; create only missing tables, repair all Payment/Receipt/ReceiptItem columns idempotently, and relax only obsolete legacy-only required columns, preserving every existing row.
**Files Affected**: `src/backend/MasterMind.API/Controllers/FeeCollectionController.cs`, `src/backend/MasterMind.API/Program.cs`, `src/backend/MasterMind.API.Tests/FinanceWriteEndpointTests.cs`
**Date Learned**: 2026-08-06
